  ...
  return ({
    ...
    pay() {
      ...
      for (let i = 0; i < this.consumption.length; i += 1) {
        if (this.consumption[i] in obj.food) {
          payArr.push(obj.food[this.consumption[i]]);
        }
        if (this.consumption[i] in obj.drink) {
          payArr.push(obj.drink[this.consumption[i]]);
        }
      }
      console.log(payArr);
      let total = payArr.reduce((prev, current) => {
        prev += current;
        return prev;
      });
      return parseFloat((total + (total * 0.1)).toPrecision(3));
    },
  });
}
